...
(function() {
  if (localStorage.getItem('kuleather_admin_theme') === 'dark') {
    ...
    const adminIndex = currentPath.indexOf('/Kuleather-Light/');
    if (adminIndex !== -1) {
      const relativePath = currentPath.substring(adminIndex + '/Kuleather-Light/'.length);
      window.location.replace(currentPath.substring(0, adminIndex) + '/Kuleather-Dark/' + relativePath + window.location.search);
    }
  }
})();

// ==========================================================================
// 🌿 Kuleather Light — Core Admin Engine
// ==========================================================================
...
const DEFAULT_PRODUCTS = [
  ...
  { id: "dompet-bifold", name: "Dompet Bifold Kulit Sapi Premium", price: 350000, rating: 4.8, reviewCount: 342, sold: 891, cat: "Barang Jadi", material: "Kulit Sapi", origin: "Magetan", badges: ["New"], photo: "photo-1614252369475-531eba835eb1", inStock: true, sku: "KUL-WAL-002", stock: 120, seller: "Toko Pak Slamet", status: "active", description: "Dompet lipat dua klasik dengan jahitan tangan presisi. Dilengkapi dengan 6 slot kartu and kompartemen uang kertas berlapisan kain suede halus.", variants: ["Cognac", "Hitam", "Dark Brown"], createdAt: "2024-02-10" },
...
function seedData() {
  ...
  if (storedProducts && (storedProducts.includes('photo-1627123424574-724758594785') || storedProducts.includes('photo-1464699908537-0954e50791ee') || storedProducts.includes('"Premium"') || storedProducts.includes('"Sale"'))) {
    localStorage.removeItem('kuleather_products');
  }
  const storedServices = localStorage.getItem('kuleather_services');
  if (storedServices && (storedServices.includes('photo-1590534244455-89f55e092147') || storedServices.includes('photo-1524388680868-377a2e6bbb1c'))) {
    localStorage.removeItem('kuleather_services');
    localStorage.removeItem('kuleather_services_version');
  }
...
window.adminSwitchTheme = function() {
  // Current folder is Kuleather-Light, switch to Kuleather-Dark
  const currentPath = window.location.pathname;
  // Determine relative page path (e.g. /admin/sellers.html)
  const adminIndex = currentPath.indexOf('/Kuleather-Light/');
  if (adminIndex === -1) return;
  const relativePath = currentPath.substring(adminIndex + '/Kuleather-Light/'.length);
  localStorage.setItem('kuleather_admin_theme', 'dark');
  // Navigate to same page in Dark folder
  const newPath = currentPath.substring(0, adminIndex) + '/Kuleather-Dark/' + relativePath;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (themeToggleContainer) {
    themeToggleContainer.innerHTML = `
      <button class="admin-topbar-btn admin-theme-toggle-btn" id="admin-theme-btn" onclick="adminSwitchTheme()" aria-label="Ganti ke Mode Gelap" title="Ganti ke Mode Gelap">
