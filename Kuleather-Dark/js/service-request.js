        ...
        if (freshOrderIdx !== -1) {
          ...
          if (ordId === 'SVO-102') {
            ...
            if (order.status === 'REQUEST_SENT') {
              ...
              order.status = 'PENAWARAN_DIKIRIM';
              order.offer = { finalPrice: budget, completionDays: 7, estimatedDays: estimasi, freeRevisions: 2, extraRevisionCost: 75000, notes: 'Kami siap memproses pesanan kustom Anda sesuai spesifikasi.' };
              order.revisions.max = 2;
              order.revisions.extraCost = 75000;
              order.negotiation.push({ from: 'seller', message: `Mengirimkan penawaran harga final sebesar ${formatIDR(budget)} dengan estimasi pengerjaan ${estimasi}. Batas revisi gratis: 2 kali (Revisi tambahan: ${formatIDR(75000)} /revisi).\nCatatan: "Kami siap memproses pesanan kustom Anda sesuai spesifikasi."`, date: new Date().toLocaleDateString('id-ID'), time: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) });
              if (!order.timeline.find(t => t.status === 'PENAWARAN_DIKIRIM')) { order.timeline.push({ status: 'PENAWARAN_DIKIRIM', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Penawaran harga dari pengrajin dikirim.' }); }
              replyText = `Halo, request kustom Anda sudah kami terima dan periksa. Kami siap mengerjakan sesuai spesifikasi yang diminta. Sudah kami kirimkan penawaran harga ${formatIDR(budget)} dengan estimasi ${estimasi}, silakan di-review.`;
              notificationMsg = `Pengrajin mengirimkan penawaran kustom untuk #${ordId}.`;
            } else if (order.status === 'PENAWARAN_DIKIRIM') {
              order.status = 'DEAL';
              ...
              if (!order.timeline.find(t => t.status === 'DEAL')) { order.timeline.push({ status: 'DEAL', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Kesepakatan harga & waktu tercapai.' }); }
              ...
              if (!order.timeline.find(t => t.status === 'DIBAYAR_ESCROW')) { order.timeline.push({ status: 'DIBAYAR_ESCROW', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Deposit escrow diamankan platform.' }); }
              ...
              order.status = 'DIKERJAKAN';
              if (!order.timeline.find(t => t.status === 'DIKERJAKAN')) { order.timeline.push({ status: 'DIKERJAKAN', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Pengerjaan fisik oleh pengrajin dimulai.' }); }
              ...
              if (order.progress.length === 0) {
                ...
                if (!order.timeline.find(t => t.status === 'HASIL_DIKIRIM')) { order.timeline.push({ status: 'HASIL_DIKIRIM', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Hasil akhir jadi diserahkan pengrajin.' }); }
              ...
              if (!order.timeline.find(t => t.status === 'SELESAI')) { order.timeline.push({ status: 'SELESAI', date: new Date().toLocaleDateString('id-ID') + ' ' + new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), text: 'Order selesai disetujui pembeli.' }); }
